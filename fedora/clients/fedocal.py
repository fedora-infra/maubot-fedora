...
class FedocalClient:
    ...
    async def future_meetings(self, num = 3):
        ...
        if response == "":
            response = f"There are no future meetings in FedoCal."
        return response
    ...
    async def next_room_meeting(self, room):
        ...
        if nextroom_meetings:
            ...
            response = f"The next meeting in this room ({room}) is: '{meeting['meeting_name']}' (starting {arrow.get(date).humanize()})"
        else:
